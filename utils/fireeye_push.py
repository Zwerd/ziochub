...
Per-target optional fields (stored in automation_fireeye_appliances JSON):
...
- delete_http_method: DELETE (default) or POST — some appliances (e.g. Trellix NX wsapis remove)
  expect POST with an empty body instead of HTTP DELETE.

Important: Some products return HTTP 200 with a JSON body indicating rejection or async failure.
We read the response body and treat known failure shapes as unsuccessful (not just status code).
"""
import json
import logging
import threading
